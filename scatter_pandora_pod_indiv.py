...
for n in range(len(pollutants)):
    ...
    for k in range(len(locations)):
        ...
        ax4.scatter(merge['{}'.format(pollutants[n])], merge['Pandora {} {}'.format(typeLabel[n],pollutants[n])],c=colors[k], s=25)

        #Final touches for plotting
        #fig4.tight_layout()  
        #Add x and y axis labels
        ax4.set_xlabel('Ground {} (ppb)'.format(pollutants[n]))
        if typeLabel[n] == 'Surface':
            ax4.set_ylabel('Pandora {} {} (ppb)'.format(typeLabel[n],pollutants[n]))
        else:
            ax4.set_ylabel('Pandora {} {} (mol/m2)'.format(typeLabel[n],pollutants[n]))
        ...
        if IQR == 'yes':
            fig4.suptitle('{} - Pandora {} IQR vs. Ground {}'.format(locations[k],typeLabel[n],pollutants[n]), y=.93)  # Adjust the vertical position (0 to 1)
        else:
            fig4.suptitle('{} - Pandora {} vs. Ground {}'.format(locations[k],typeLabel[n],pollutants[n]), y=.93)  # Adjust the vertical position (0 to 1)
        #Add a 1:1 line
        plt.plot([min(merge['{}'.format(pollutants[n])]), max(merge['{}'.format(pollutants[n])])], [min(merge['Pandora {} {}'.format(typeLabel[n],pollutants[n])]), max(merge['Pandora {} {}'.format(typeLabel[n],pollutants[n])])], color='black', linestyle='--', label='1:1 Line')
        ...
        if IQR == 'yes':
            savePath = os.path.join(Spath,'PandoraIQR_ground_scatter_{}_{}'.format(locations[k],pollutants[n]))
        else:
            savePath = os.path.join(Spath,'Pandora_ground_scatter_{}_{}'.format(locations[k],pollutants[n]))
